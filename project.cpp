...
{
	strcpy(emp.ID,"0123456789");
	
	strcpy(emp.pass,"12345678");
	...
	void create_acc(struct Bank_acc *users )
{
	...
	std::cin.get(users[n].name,30);
	std::cin.ignore();

	std::cout<<"please enter your address"<<std::endl;
	std::cin.get(users[n].address,30);
	std::cin.ignore();
	...
	std::cin.get(users[n].G_national_id,14);
	std::cin.ignore();
		for(int i=0;i<10;i++)
		{	
		 int j=9-i;
		 users[n].id.ID[i]=users[n].G_national_id[j];
	...
	std::cin.get(users[n].national_id,14);
	std::cin.ignore();
		for(int i=0;i<10;i++)
		{	
		 int j=9-i;
...
void open_acc(struct Bank_acc *users,struct Bank_employee emp )
{
	...
	std::cin.ignore();
	std::cout<<"Enter your emp ID is"<<emp_id<<std::endl;;
	
	std::cout<<"Enter your emp pass"<<std::endl;
	std::cin.get(emp_pass,9);
	std::cin.ignore();
	std::cout<<"Enter your emp pass is"<<emp_pass<<std::endl;
...
int main()
{
	...
	while (1)
	{
		std::cout<<"choose the feature you want"<<std::endl<<"1-admin window"<<std::endl<<"2-client window"<<std::endl;
		std::cin>>feature;
		std::cin.ignore();
		switch(feature)
		{
